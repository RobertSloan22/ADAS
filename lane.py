...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (407,193), # Top-left corner
      (195, 387), # Bottom-left corner            
      (745,386), # Bottom-right corner
      (539,193) # Top-right corner
    ])
         
    # The desired corner locations  of the region of interest
    # after we perform perspective transformation.
    # Assume image width of 600, padding == 150.
    self.padding = np.int32(0.25 * width) # padding from side of the image in pixels
...
def main():
    # Initialize video capture object to capture video from the default webcam
    ...
    while True:
        ret, frame = cap.read() 
        _, frame = cap.read()
        #frame = cv2.flip(frame, -1)# Read a frame from the video capture object
        if not ret:
        ...
        lane_obj.plot_roi(plot=False)  # Set to True if you want to see the ROI on each frame
        warped_frame = lane_obj.perspective_transform(plot=False)  # Set to True to plot
        histogram = lane_obj.calculate_histogram(plot=False)  # Set to True to plot
        left_fit, right_fit = lane_obj.get_lane_line_indices_sliding_windows(plot=False)  # Set to True to plot
        lane_obj.get_lane_line_previous_window(left_fit, right_fit, plot=False)  # Set to True to plot
        frame_with_lane_lines = lane_obj.overlay_lane_lines(plot=False)  # Set to True to plot
        lane_obj.calculate_curvature(print_to_terminal=False)  # Set to True to print
        lane_obj.calculate_car_position(print_to_terminal=False)  # Set to True to print
        frame_with_lane_lines2 = lane_obj.display_curvature_offset(frame=frame_with_lane_lines, plot=False)  # Set to True to plot

        # Display the processed frame
        cv2.imshow('Lane Detection', frame_with_lane_lines2)
