...
from jetson_utils import (videoSource, videoOutput, cudaAllocMapped, cudaConvertColor, cudaMemcpy, cudaDeviceSynchronize, cudaToNumpy)
...
current_camera_path = "v4l2:///dev/video1"
...
class Lane:
    ...
    def get_line_markings(self):
        # Allocate GPU memory and upload the original frame
        gpu_frame = cudaAllocMapped(width=self.orig_frame.shape[1], height=self.orig_frame.shape[0], format='rgb8')
        cudaMemcpy(gpu_frame, self.orig_frame)

        # Convert to grayscale using CUDA
        gray_gpu = cudaConvertColor(gpu_frame, 'rgb8', 'gray8')

        # Apply Gaussian Blur using CUDA (Approximate as Resize Blur)
        blurred_gpu = cudaFilter(gray_gpu, filter='gaussian')

        # NOTE: Since jetson_utils does not support Canny Edge Detection,
        # we can use an approximation using Sobel filters instead.
        # Download the blurred image back to CPU to use OpenCV Canny.
        blurred_cpu = cudaToNumpy(blurred_gpu)

        # Perform Canny edge detection using OpenCV after converting back to CPU
        edges = cv2.Canny(blurred_cpu, 50, 150)

        # Upload the edge map back to GPU
        gpu_edges = cudaAllocMapped(width=edges.shape[1], height=edges.shape[0], format='gray8')
        cudaMemcpy(gpu_edges, edges)

        # Keep the result in GPU memory for further processing or return as needed
        self.lane_line_markings = gpu_edges
