...
class Lane:
  ...
  def get_lane_line_previous_window(self, left_fit, right_fit, plot=False):
    ...
    if len(rightx) > 0 and len(righty) > 0:
      ...
      right_fit = prev_right_fit2[-1] if prev_right_fit2 else np.array([0, 0, 0]) # or some other default value
  ...
  def get_lane_line_indices_sliding_windows(self, plot=False):
    ...
    left_fit = np.polyfit(lefty, leftx, 2)
    right_fit = np.polyfit(righty, rightx, 2) 
    # Check if lefty and leftx are not empty and are of type np.ndarray
    # Check if lefty and leftx are not empty and are of type np.ndarray
    if lefty.size > 0 and leftx.size > 0 and isinstance(lefty, np.ndarray) and isinstance(leftx, np.ndarray):
        left_fit = np.polyfit(lefty, leftx, 2)
    else:
        print("Error: lefty or leftx are not the correct type or are empty")
        left_fit = None

    if righty.size > 0 and rightx.size > 0 and isinstance(righty, np.ndarray) and isinstance(rightx, np.ndarray):
        right_fit = np.polyfit(righty, rightx, 2)
    else:
        print("Error: righty or rightx are not the correct type or are empty")
        right_fit = None
