...
def gen_frames(): 
    while True:
        ...
        for class_id, score, bbox in zip(class_ids, scores, bboxes):
            ...
            distances.sort()
            nearest_cars = distances[:5]  # Get the three nearest cars
            ...
            if class_name in active_buttons:
                cv2.putText(frame, class_name, (x, y - 10), cv2.FONT_HERSHEY_DUPLEX, 2, BLUE, 3)
               ...
               # for angle in range(0, 360, 90):
                ...
                if class_name in ["person", "car", "truck", "bus", "motorbike", "stop sign"]:
                    ...
                    if distance_in_feet < 50:  # Adjust this threshold as needed
                        ...
                        cv2.putText(frame, distance_message, (40, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
                        if distance_in_feet < 15:
                            alert_message = "PROXIMITY ALERT".format(class_name, round(distance_in_feet, 2))
                            cv2.putText(frame, alert_message, (int(frame.shape[1] / 2 - 150), int(frame.shape[0] / 2)), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)
