...
class Lane:
 ...
 # define the global variable prev_left_fit and prev_right_fit
    ...
    self.roi_points = np.float32([
      (274,384), # Top-left corner
      (150, 537), # Bottom-left corner            
      (775,537), # Bottom-right corner
      (571,184) # Top-right corner
    ])
  ...
  # Extract the pixel coordinates for the left and right lane lines
    ...
    # Check if rightx is not empty before calling np.polyfit
 # or some default value
    # Use global variables to access the previous polynomial fits
    global prev_left_fit, prev_right_fit

    # Initialize left_fit and right_fit
    left_fit = None
    right_fit = None

    # Only attempt to fit a polynomial if there are enough pixels detected
    if leftx.size > 0 and lefty.size > 0:
        left_fit = np.polyfit(lefty, leftx, 2)
    elif 'prev_left_fit' in globals():
        left_fit = prev_left_fit  # Use the previous fit if available

    if rightx.size > 0 and righty.size > 0:
        right_fit = np.polyfit(righty, rightx, 2)
    elif 'prev_right_fit' in globals():
        right_fit = prev_right_fit  # Use the previous fit if available

    # Update the previous fits
  ...
  def perspective_transform(self, frame=None, plot=False):
    ...
    if plot == True:
      ...
      warped_plot = cv2.polylines(warped_copy, np.int64([
  ...
  def plot_roi(self, frame=None, plot=False):
    ...
    this_image = cv2.polylines(frame, np.int64([
...
def gen_frames():
  ...
  while cap.isOpened():
    ...
    if ret == True:
      ...
      # q == quit
      ret, buffer = cv2.imencode('.jpg', frame_with_lane_lines2)
