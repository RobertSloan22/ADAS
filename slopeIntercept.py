...
def make_coordinates(image, line_parameters, side):
    try:
        ...
        slope = 0.3 if side == 'left' else -0.4
...
def gen_frames(): 
    while True:
        ...
        lines = cv2.HoughLinesP(cropped_image, 2, np.pi/180, 100, np.array([]), minLineLength=50, maxLineGap=20)
        if lines is not None:
        ...
        for class_id, score, bbox in zip(class_ids, scores, bboxes):
            ...
            if class_name in active_buttons:
                cv2.putText(frame, class_name, (x, y - 10), cv2.FONT_HERSHEY_DUPLEX, 1, BLUE, 3)
                ...
                cv2.circle(frame, center, radius, BLUE, thickness=3)
                ...
                if class_name in ["person", "car", "truck", "bus", "motorbike", "stop sign"]:
                    ...
                    if distance_in_feet < 50:  # Adjust this threshold as needed
                        ...
                        cv2.putText(frame, alert_message, (x, y - 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)
                        if distance_in_feet < 15:
                            alert_message = "{} is {} FT".format(class_name, round(distance_in_feet, 2))
                            cv2.putText(frame, alert_message, (int(frame.shape[1] / 2 - 150), int(frame.shape[0] / 2)), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
